...
from db import create_db, query_nearby
...
def pick_true_outpost(conn, flashes, attempts=200, jitter_km=8.0):
    ...
    for attempt in range(attempts):
        ...
        fingerprint = build_fingerprint(candidate, rows, min_flashes=5)
        if fingerprint is not None:
            debug(
                f"pick_true_outpost: attempt={attempt} anchor=({anchor.lat:.4f},{anchor.lon:.4f}) "
                f"candidate=({candidate.lat:.4f},{candidate.lon:.4f}) nearby_rows={len(rows)} "
...
def main(seed=None):
    ...
    conn = create_db(flashes)
    debug(f"storm_radius_km={storm_radius_km:.4f}")

    true_outpost, recorded_fingerprint = pick_true_outpost(conn, flashes)

    # Solver only ever sees recorded_fingerprint and conn, not true_outpost.
    calculated_outpost = estimate_outpost(conn, recorded_fingerprint, debug_fn=debug)
